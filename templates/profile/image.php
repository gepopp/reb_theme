...
<div class="col-span-3 lg:col-span-1">
    <form enctype="multipart/form-data" class="bg-black text-white shadow-md px-8 pt-6 pb-8 mb-4 h-full" method="post" action="<?php echo admin_url('admin-post.php') ?>"
          x-data="profileImage('<?php echo $image['sizes']['thumbnail'] ?? false ?>')"
          @submit.prevent="submit()"
          x-ref="form">
        <h1 class="text-3xl font-bold mb-5 uppercase"><?php _e('Your Avatar', 'reb_domain') ?></h1>
        ...
        <div class="flex flex-col items-center justify-center">
            ...
            <button class="bg-logo text-white font-bold py-2 px-4 block w-full focus:outline-none focus:shadow-outline" x-show="!imageUrl"
                    @click="$refs.upload.click()"
                    type="button">
                <?php _e('Upload', 'reb_domain') ?>
            </button>
            <button class="bg-primary-100 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" x-show="imageUrl"
                    type="submit">
                <span x-show="!isLoading"><?php _e('save', 'reb_domain') ?></span>
                <span x-show="isLoading" class="flex space-x-2 items-center">
                    ...
                    <span><?php _e('loading', 'reb_domain') ?></span>
                </span>
            </button>
        </div>
    </form>
